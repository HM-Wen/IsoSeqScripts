...
EDL_7KB = ['7-15kb_grp3_edl_8cell','7-15kb_grp4_edl_8cell']
CAR_7KB = ['7-15kb_grp5_cardiac_6cell','7-15kb_grp6_cardiac_5cell']
...
def printClustKeyDict(clustKeyDict, mergedKeyPath):
    ...
    with open(mergedKeyPath, 'wb') as mergedKeyOutFile:
        ...
        for fastq, clustDict in clustKeyDict.iteritems():
        ...
        for fractionName in mergeWithin:
            if fractionName =='3-6kb':
                for groupList in THREE:
                    print "Merging tissue files within {0}...".format(fractionName)
...
def getMergedWithinTupLists(mergeAcross, resultsRoot, dirPolyA):
    ...
    # expect one match each
        solReport = glob.glob(os.path.join(sizeDir, '*soleus*report.csv'))
        solFastq = glob.glob(os.path.join(sizeDir, '*soleus*.fastq'))

        edlReport = glob.glob(os.path.join(sizeDir, '*edl*report.csv'))
        edlFastq = glob.glob(os.path.join(sizeDir, '*edl*.fastq'))

        carReport = glob.glob(os.path.join(sizeDir, '*cardiac*report.csv'))
        carFastq = glob.glob(os.path.join(sizeDir, '*cardiac*.fastq'))
